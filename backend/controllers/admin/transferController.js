...
const approveTransferRequest = async (req, res) => {
  ...
  try {
    ...
    let transferResult;
    try {
      transferResult = await blockchainService.transferTicket({
        from: normalizeAddress(transfer.from_wallet_address),
        to: normalizeAddress(transfer.to_wallet_address),
        tokenId: String(ticket.blockchain_ticket_id),
      });
    } catch (err) {
      await connection.query(
        `
