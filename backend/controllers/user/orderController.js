...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    let { items, notes, payment_method, answers } = req.body;
    ...
    const eventId = ticketTypeRows[0].event_id;
    ...
    // =========================
    // Validate event questions and answers
    // =========================
    ...
    for (const answer of answers) {
      ...
      if (Number.isNaN(questionId) || questionId <= 0) {
        ...
        return res.status(400).json({
          success: false,
          message: "question_id trong answers không hợp lệ",
        });
      }
      ...
      if (answerMap.has(questionId)) {
        ...
        return res.status(400).json({
          success: false,
          message: `Câu hỏi ID ${questionId} bị trả lời trùng`,
        });
      }
    ...
    for (const question of questionRows) {
      if (Number(question.is_required) === 1) {
        ...
        if (!answerText) {
          ...
          return res.status(400).json({
            success: false,
            message: `Bạn chưa trả lời câu hỏi bắt buộc: ${question.question_text}`,
          });
        }
      }
    }
    ...
    for (const answer of answers) {
      ...
      const matchedQuestion = questionRows.find((q) => q.id === questionId);

      if (!matchedQuestion) {
        await connection.rollback();
        return res.status(400).json({
          success: false,
          message: `Câu hỏi ID ${questionId} không thuộc sự kiện này`,
        });
      }
    }
    ...
    for (const answer of answers) {
      ...
      await connection.query(
        `
        INSERT INTO order_question_answers (
          order_id,
          question_id,
        ...
        VALUES (?, ?, ?, ?, NOW())
        `,
        [orderId, questionId, matchedQuestion.question_text, answerText]
      );
    }
    ...
    const [answerRows] = await pool.query(
      ...
      SELECT
        ...
        oqa.order_id,
        oqa.question_id,
        oqa.question_text_snapshot,
        eq.question_type,
        oqa.answer_text,
        oqa.created_at
      FROM order_question_answers oqa
      INNER JOIN event_questions eq ON eq.id = oqa.question_id
      WHERE oqa.order_id = ?
      ORDER BY oqa.id ASC
      `,
      [orderId]
